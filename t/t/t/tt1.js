/*
console.show()
log(new Date("2021/06/16 14:19:50 +0000 +0000"))
exit()
/** */
"ui";
// auto.setMode("fast");
...
tempSave.showLoginTool = tempSave.test ? "true" : "false";
{
    let logs = [
        ...
        "新增强制关注满的选项", // 2021年6月14日 17:45:36
        "修复有时账号异常还继续跑运行", // 2021年6月14日 22:44:46
        "修复关注模式首次加载用户失败导致直接结束", // 2021年6月15日 14:09:23
        "优化上传视频授权速度及适配低分辨率", // 2021年6月16日 16:19:59
        "新增key上号前使用appenv修改环境", // 2021年6月17日 00:27:59
        "尝试修复图库选头像时在大图卡住", // 2021年6月17日 00:28:37
        "测试_关注模式打开用户失败时换一个用户打开",
        "修复全屏图片无法选择，全屏视频无法选择", // 2021年6月17日 13:23:45
        "修复上传头像异常停止", // 2021年6月17日 14:43:45
        "修复视频上传，优化关注模式的检测等待时长", // 
        "优化视频上传，优化检测用户打开状态", // 2021年6月18日 18:01:33
        "修复关注模式打开链接后报错", // 2021年6月18日 23:48:11
        "优化打开链接时的等待时间，最大30s", // 2021年6月19日 00:23:07
        "修复因为找不到Follow按钮而导致异常", // 2021年6月19日 14:13:56
        "测试_关注用户", // 
    ];
        ...
        {/* text="模式选择"/> */}
                            ...
                            <vertical id="loginmodel">
                                ...
                                <linear padding="2 0 0 0">
                                    <checkbox id="functionTest" text="测试模式" />
                            ...
                            <vertical id="updatemodel">
                                ...
                                <linear padding="5 0 0 0">
                                    ...
                                    <input lines="1" id="gzjg" w="auto" text="200" />
                                    <text textColor="black" text="~" />
                                    <input lines="1" id="gzjg1" w="auto" text="800" />
                                    <text textColor="black" text="ms" />
                                </linear>
                                <linear padding="5 0 0 0">
                                    <checkbox id="failureContinue" text="关注失败也继续关注" />
                                </linear>
                                <linear padding="5 0 0 0">
...
function 主程序(forTag) {
    ...
    if(ui.functionTest.checked) {
        ...
        let cf = true;
        ui.run(()=>{
            cf = floaty.rawWindow(<frame><button id="but">开始测试</button></frame>)
...
{
    let t = 300;
    let words = ["Follow","Message","Requested","Edit profile","Set up profile"];
/*
    1. 获取不大那个按钮的时候采用id获取
    2. 关注完成不会切号
    3. 
...
/*
    循环执行([
        step(
            "全屏视频"
            , function() {return (this.uo = text("Followers").findOne(1000))}
            , function(){
                let follow = clickable(true).filter(function(uo){
                    if(uo.text() == "") return false;
                    console.info(words.indexOf(uo.text()), uo.text())
                    if(-1 < words.indexOf(uo.text())) {
                        log(uo.drawingOrder(),uo.className())
                        log((uo.drawingOrder() == 0 
                            || uo.drawingOrder() == 1
                        ), (
                            uo.className() == "android.widget.TextView"
                            || uo.className() == "android.widget.Button"
                        ))
                    }
                    if((uo.drawingOrder() == 0 
                        || uo.drawingOrder() == 1
                    ) && (
                        uo.className() == "android.widget.TextView"
                        || uo.className() == "android.widget.Button"
                    )) {
                        console.warn(uo.clickable())
                        return -1 < words.indexOf(uo.text());
                    }
                    return false;
                }).find()
                // log(follow)
                console.info(follow.length)
console.error(
    text("Follow").findOne(1000),
    id("c_e").depth(16).findOne(1000)
    )
            }
        )
        , step("跳出", ()=>{return true}, ()=>{return "跳出循环执行"})
    ])
    /** */
    let uo = id("c_e").clickable(true).findOne(1000)
    log(uo)
            log(
                "点击结果：", 
                // tempSave.version
                text("Swipe up for more").findOne(100)
                // getFansInfo("个人信息", true)
                // clickOn(uo)
                // text("Follow").findOne(1000)
            )
}
// =================================================================================
    ...
    while(true) {
        ...
        try{
            sm停止TikTok();
            if(runTikTok() && accountInfo.name && accountInfo.name != -1) {
                log("账号正常，还原成功", accountInfo)
                // 开启一个新线程来保存账号
                threads.start(function () {
        ...
        if (r) {
            ...
            if (打开抖音()) {
                ...
                if(accountInfo.url != ui.wz.text()){
                    log("修改链接")
                    let 编辑信息 = textContains(" profile").visibleToUser().findOne(2000)
                    if(编辑信息){
                        编辑信息.click()
                        // 修改链接
...
function 关注结果检测(){
    if(tempSave.accountKey && tempSave.accountKey.keyName) {
        ...
        if(accountInfo.focusNumber - accountFocusNumber < 10) {
            // log("重新安装TikTok", reinstallTikTok())
        }
    } else {
        返回首页();
    ...
    if(tempSave.login){
        ...
        for (let i = 0; i < 5; i++) {
            ...
            if (packageName(appPackage).findOne(2000)){
                ...
                if(textContains(" profile").findOnce()){
                    log("已经存在账号")
                    return false;
                }
                // log提示语句
...
function mi6关注操作(num) {
    ...
    // 打开链接
    if(!openUrlAndSleep3s(user.url, user)){
        mi6关注操作(计数);
    }
    ...
    if (粉絲) {
        ...
        if (!按讚) {
            ...
            while (计数 < 限制) {
                ...
                // 当勾选了失败继续关注时就让这里不生效
                sleep(500)
                if(!ui.failureContinue.checked && 4 < 关注.length && 关注.length == text("Follow").visibleToUser().find().length) {
                    tlog(ui.failureContinue.checked, 关注.length, text("Follow").visibleToUser().find().length)
                    log("关注失败", ++关注失败次数)
                    // 切换账号
                    let click = false;
                    if(3 <= 关注失败次数) {
...
function 返回首页(dayleTime) {
    log("返回首页中..")
    dayleTime = dayleTime||1000;
    for (var i = 0; i < 10; i++) {
        popupDetection(dayleTime*0.2);
        if (packageName(appPackage).exists()) {
            sleep(dayleTime*0.5)
            if (text("Me").visibleToUser().exists()) {
                break
            } else {
    ...
    for (let i = 0; i < 3; i++) {
        while(focusNumber < max) {
            ...
            try{
                ...
                } else if (state && -1 < state.text().indexOf("profile")) {
                    // 直接跳出，进度减一
                    focusNumber--;
                } else {
...
function 上传视频() {
    ...
    let 选择后操作 = [
        step_clickName("Post Now")
        , {
            标题: "编辑框",
            uo: null,
            检测: function() {
                this.uo = classNameEndsWith("EditText").visibleToUser().findOne(t)
                return this.uo
            },
            执行: function() {
        ...
        },
        {
            ...
            uo: null,
            检测: function() {
                this.uo = text("Select cover").clickable(true).findOne(t)
                return this.uo
            },
            执行: function() {
        ...
        },
        {
            ...
            uo: null,
            检测: function() {
                this.uo = text("Save").clickable(true).findOne(t)
                return this.uo
            },
            执行: function() {
                let re = this.uo.click();
        ...
        },
        {
            ...
            num: 0,
            检测: function() {
                this.uo = text("Me").visibleToUser().findOne(t)
                return this.uo
            },
            执行: function() {
                console.error("当前是主页！")
    ...
    let 选择操作 = [
        {
            ...
            uo: null,
            检测: function() {
                this.uo = text("All").visibleToUser().findOne(t)
                return this.uo
            },
            执行: function() {
        ...
        },
        {
            ...
            uo: null,
            检测: function() {
                this.uo = text("视频列表").visibleToUser().findOne(t)
                return this.uo
            },
            执行: function() {
                let re = this.uo.parent().click();
        ...
        },
        {
            ...
            uo: null,
            检测: function() {
                this.uo = text("Videos").visibleToUser().findOne(t)
                return this.uo
            },
            执行: function() {
                let re = this.uo.parent().click();
        ...
        step(
            "视频列表"
            , function () {return (this.uo = text("视频列表").findOne(t))}
            , function(){
                // 深度 7 等于已经选择
                ...
                // let re = imgUO.click()
                let re = clickOn(rv.children()[randomNumber])
                log("视频点击结果",re)
                if (false && re) {
                    // 2s 可能会慢一点
                    ...
                    for (let next = 0; next < 5; next++) {
                        // 在视频开启全屏时进行选择的，可以简写成 log("选择视频", clickOn(text("Select"))))
                        下一步 = text("Select").findOne(100);
                        if(下一步){
                            log("选择视频", clickOn(下一步))
                        }
                        // 点击 next 下一步的
                        下一步 = null;
                        下一步 = text("Next").visibleToUser().clickable(true).findOne(3000);
                        if(下一步) {
                            log("下一步 " + 下一步.click());
                            sleep(2000);
                            next--;
                            ...
                            // 低分辨率时点击箭头的 → 
                            tlog("view" + clickOn(classNameEndsWith("view.View").clickable(true).visibleToUser()))
                        ...
                        下一步 = text("CANCEL").findOne(200);
                        if(下一步) {
                            clickOn(下一步);
                        }
                        if(text("Post").findOne(50)) {
                            break;
                        }
                    }
                    if(text("Post").findOne(1000)){
                        console.info("选择完毕，进入下一阶段")
                        循环执行(选择后操作)
                        console.info("第二阶段结束")
                        return "跳出循环执行";
                    }else{
                        console.info("选择视频失败，返回操作");
                        back()
                    }
                }
            }
        ...
        step(
            "全屏视频"
            , function() {return (this.uo = text("Select").findOne(t) || text("Selected").findOne(t))}
            , function(){
                if(this.uo.text() == "Select") {
                    let list = this.uo.parent().children()
                    let uo;
                ...
                if((this.uo = text("Selected").findOne(t))){
                    // 获得当前控件
                    this.uo = this.uo.parent().children().pop();
                    // 检查有几个了
                    tlog("已选中视频个数", this.uo.text())
                    if(1 < parseInt(this.uo.text())) {
                        // 取消多选择的那个视频
                        clickOn(this.uo);
                    }
                }
        ...
        ), 
        {
            ...
            uo: null,
            检测: function() {
                this.uo = text("Next").findOne(t)
                return this.uo
            },
            执行: function() {
                log("点击" + this.标题, clickOn(this.uo))
                for (let next = 0; next < 5; next++) {
                    this.uo = text("Next").findOne(t)
                    if(this.uo) {
                        if(clickOn(this.uo)){
        ...
        step(
            "其它情况"
            , function(){
                return (this.uo = filter(function(uo){
                    // 如果遇到文字 Normal 就判定
                    return uo.text() == "Normal";
                }).findOne(t))
            }
            , function() {
                // 按下返回键
        ...
        step(
            "下一步操作"
            , function(){return (this.uo = text("Post").findOne(t) || text("Post Now").findOne(t))}
            , function(){
                console.info("选择完毕，进入下一阶段")
                循环执行(选择后操作);
                console.info("第二阶段结束")
    ...
    let 打开视频列表操作 = [
        ...
        },
        {
            ...
            uo: null,
            检测: function() {
                this.uo = filter(function(uo){
                    return uo.text() == "ALLOW" || uo.text() == "Allow" || uo.text() == "允许";
                }).findOne(t);
                return this.uo
            },
            执行: function() {
        ...
        },
        {
            ...
            uo: null,
            检测: function() {
                this.uo = text("Upload").visibleToUser().findOne(t)
                return this.uo
            },
            执行: function() {
...
function 更换头像() {
    ...
    let 图库选头像 = [
        ...
        step(
            "全屏图片"
            , function(){return text("Back").findOne(1000)}
            , function(){
                // 点击右上角的选择
                log("选择图片", clickOn(className("android.view.View").visibleToUser().clickable(true)))
            }
        ),
        {
            标题: "确定",
    ...
    let 操作 = [
        ...
        },
        {
            ...
            uo: null,
            检测: function() {
                return text("All media").visibleToUser().findOne(500) || text("Back").visibleToUser().findOne(500);
...
// ！！！！！！！ 网络问题导致加载不出来粉丝信息从而卡死。 进入后不知道点了哪里出现好几个选项的面板，其中一个是Message。
                        //  发送私信
...
function sayHello(f, msg){
    ...
    let 操作 = [
        ...
        , step(
            "超时判定"
            , function() {
                console.error(nowTime < Date.now(), nowTime, Date.now()); 
...
function clcikSendMsgUiObject(startHeight){
    // 获取到输入框
    let inUO = className("android.widget.EditText").clickable(true).findOne(1000);
    // 用输入框的爷爷控件找输入框的图片舅舅
    let sendUO = inUO.parent().parent().findOne(className("android.widget.ImageView").depth(inUO.depth() - 1));
    return clickOn(sendUO);
...
/*             // 4. 检测是否发送成功 
        ...
        if(m.status) {
            ...
            if(msg.indexOf("{s}") < 0) {
                ...
                // 发送第N条消息
                return sendMsg(msg.substring(msg.indexOf("{s}") + "{s}".length + 1), sayHelloTag,
                        null/* 重置发送次数 */, emojiData[random(0, emojiData.length-1)])
            }
        }
...
function 修改邮箱(email) {
    ...
    let 操作 = [
        ...
        step(
            "在主页打开设置"
            , function(){return (this.uo = text("Me").findOne(t))}
            , null
            , function(){
                let 右上角设置 = classNameEndsWith("RelativeLayout").drawingOrder(7).clickable(true).findOne(t*2);
                if(右上角设置) {
                    clickOn(右上角设置);
                }
            }
        )
        , step_clickName("Manage account")
        , step(
            "检测是否需要验证邮箱"
            , function(){
                this.uo = text("Verify your email").findOne(t);
                return text("Manage account").findOne(t) 
                    && text("Account information").findOne(t);
            }
            , function() {
                if(this.uo){
                    // 未绑定邮箱
                    log("换绑邮箱", clickOn(text("Email")));
                } else {
                    // 已绑定邮箱
                    console.error("已绑定邮箱");
                    return "跳出循环执行";
        ...
        , step_clickName("Change email address")
        , step(
            "更改邮箱"
            , function () {return (this.uo = text("Send code").findOne(t))}
            , function(){
                // 输入邮箱
                if(setText(email)){
                    // 发送验证码
                    if(clickOn(text("Send code"))) {
                        // text = Enter a valid email address
                        let sendCodeException = {
                            "Enter a valid email address": ()=>{log("邮箱无效")}
                            , "No network connection": ()=>{log("网络异常")}
                            , "Resend code": ()=>{
                                输入验证码(email);
                                return "跳出";
                            }
                        ...
                        // 查找控件
                        while (1) {
                            let eUO = filter(function(uo){
                                for (let key in sendCodeException) {
                                    if(uo.text() == key){
                                        return true;
                                    }
                                }
                            ...
                            tlog(eUO.text())
                            if(sendCodeException[eUO.text()]() == "跳出") {
                                return "跳出循环执行";
                            }
                        }
                    } else {
...
// 去修改密码，由于和邮箱很像，所以可以让其有共同的地方，如果检测失败才返回首页
/**
 * 
 * @param {String} email 要修改密码的邮箱，如果需要更改邮箱的话就使用这个邮箱来更改
 */
function 修改密码(email, password) {
    password = password || rondomPassword();
    let t = 300;
    let 操作 = [
        // 打开设置
        step(
        ...
        , step(
            "更改邮箱"
            , function () {return (this.uo = text("Resend code").findOne(t))}
            , function(){
                输入验证码(email, 60);
            }
        )
        , step(
            "设置密码"
            , function () {return (this.uo = text("Create password").findOne(t))}
            , function(){
                log("设置密码", password);
                while (1) {
                    let inPD = className("android.widget.EditText").findOne(500);
                    if(inPD) {
                        inPD.setText(password);
                        clickOn(text("Next"));
                    }
                    if(text("Email").findOne(50) && text("Password").findOne(50)){
                        // 上传账号密码
    ...
    function 输入验证码(email, secondTime){
        let time = t || 300;
        log("等待验证码", email);
        while (1) {
            // 输入验证码
            if(输入脆球邮箱(email, secondTime)) break;
            // 重新获取验证码
            clickOn(text("Resend code"));
            // 返回
            if(text("Manage account").findOne(time) 
            && text("Create password").findOne(time)
            && text("Account information").findOne(time)) {
                // 已成功返回时
                break;
            }
        }
...
/**
 * 在输入验证码界面输入验证码
 * @param {String} email 要接码的邮箱
 * @returns 
 */
function 输入脆球邮箱(email, secondTime) {
    let code = 脆球接码(email, secondTime);
    if (code) {
        if (setText(code)) {
            let recode = true;
            // 检测验证码 text = Email verification code error
            for (let i = 0; i < 10;) {
                if (filter(function (uo) {
                    return uo.text() == "Enter 6-digit code"
                        || uo.text() == "Email verification code error"
                        || uo.text() == "Email verification code expired"
                        || uo.text() == "No network connection";
                }).findOne(500)) {
                    log("重试...")
                    // 重新获取
                    sleep(1000);
 ...
 * @param {String} email 要接码的邮箱
 */
function 脆球接码(email, secondTime){
    if( !(3 < secondTime)) {
        // secondTime 不是数字或者小于3秒时
        secondTime = 300;
    }
    let subject = "is your verification code";
    let body = {
        token: "35949fd27bb2403ea43c208bc2db351b"
        , mail_id: "x3W3JO15eb"
        // , start_time: new Date().toLocaleString().substring(0,9).replace(/\//g,"-")
    ...
    while (!code) {
        log("收取验证码中");
        let respData = http.post("http://wuxianyouxiang-open-api.cuiqiu.com/v1/box/list", body)
        let jsonData = respData.body.json();
        if(jsonData.data.list) {
            log(jsonData.data.list.length + "封邮件")
            // 需要存在前几分钟的验证码
            let mail = jsonData.data.list.shift();
            tlog("邮件信息"
                , "当前时间：" + new Date().toTimeString()
                , "邮件时间：" + new Date(mail.time.replace(/-/g,"/")).toTimeString()
                , "选择的邮件：" + mail)
            // new Date() - new Date("2021/06/16 14:19:50 +0000 +0000")
            if((new Date() - new Date(mail.time.replace(/-/g,"/"))) < (secondTime * 1000)) {
                code = mail.subject.substring(0,mail.subject.indexOf(subject));
...
function 脆球邮箱生成(){
    let email = newAccount("@bosslee888.com")
    
    // while (1) {
    //     // 拿这个邮箱去查询是否存在，如果存在则重新生成
    // }
 ...
 * @returns {String} 密码
 */
function rondomPassword(){
    let pdLen = 10, words = 'abcdefghijklmnopqrstovwxyz'.split("");
    let pd = "" + words[random(0, words.length-1)].toLocaleUpperCase();
    
    for (let i = 0; i < pdLen; i++) {
        if(pdLen-i < 3) {
            pd += random(0, 9);
        } else{
            pd += words[random(0, words.length-1)]
        }
    }
    return pd;
}
...
function 等待加载(s,num) {
    ...
    sleep(s||2000)
    for (; i < num; i++) {
        if(!className("android.widget.ProgressBar").visibleToUser()
        .boundsInside(device.width*0.4,device.height*0.4,device.width*0.6,device.height*0.6)
        .findOnce())
            if(id("ca_").visibleToUser().boundsInside(0,0, device.width,device.height).find().length==0) 
                break
...
function clickOn(getUOFun, sleepTime, parentNumber, rectScope) {
    // 0. 判空
    if(typeof getUOFun == "undefined" || getUOFun == null) {
        console.verbose("参数不存在");
        return false;
    }
    //// 初始化
    // 创建对象
...
function runTikTok(run,tag) {
    ...
    if(!app.launch(appPackage)) {
        // 安装tiktok
        if(installTiktok()){
            log("安装完成，尝试再次启动", app.launch(appPackage))
        }
...
function openUrlAndSleep3s(url, user, noWait) {
    ...
    let re = dfs(words, true);
    log("链接打开结束", noWait ? "不等待" : re)
    return re;
}
function dfs(words, wait, time) {
    // let result = 0,
    let result,
        t = 100, userInfoFlag = ["Following", "Followers", "Likes"].concat(words)
        thatTime = Date.now(), /* 最大等待时间 */ maxTime = 30 * 1000, stepTime = time || 10 * 1000;  // stepTime 打开链接所等待的时间
    let 操作 = [
        step(
            "等待加载"
            , function(){
                ...
                let waitFlag = true;
                // 超时转一遍过来之后进行退出
                if(maxTime < Date.now()-thatTime) return true;
                // 开启一个线程去检测等待加载
                startThreads(()=>{等待加载(); waitFlag = false;});
                // 当前线程死循环等待标记解除
                while (waitFlag) {
                    if(maxTime < Date.now()-thatTime) {
                        // 如果超时则直接跳出,但是还要再转一遍，不能走后面的计时增加
                        return false;
                    }
                    // 死循环每次等待 500ms
                    sleep(500);
                }
                tlog("等待加载信息", stepTime, Date.now() - startTime);
                stepTime += Date.now() - startTime;
            }
            , function(){
                console.error("加载超时！");
                return "跳出循环执行"
        ...
        , step(
            ...
            , function(){
                ...
                let follow = className("android.widget.TextView")
                            .clickable(true).filter(function(uo){
                                if(uo.drawingOrder() == 0 || uo.drawingOrder() == 1)
                                return -1 < words.indexOf(uo.text());
                                retunr false;
                            }).find()
                if (follow.length == 0) {
                ...
                // 备用方案 如果存在其它文字的话就跳出 Following Followers Likes
                if(2 < filter(function(uo){
                        return -1 < userInfoFlag.indexOf(uo.text())
                    }).find().length) {
                        // 用户信息中间的那个按钮
                        let but = id("c_e").clickable(true).findOne(t);
                        if(!but){
                            console.warn("未获取到中间的按钮");
                        } else {
                            log("按钮可能无法点击");
                            but.setText("Follow");
                            result = but;
                        }
                        // 进入"执行"操作，也就是跳出整个循环执行
                        return true;
                }

                if(stepTime < Date.now()-thatTime) {
                    console.error("打开超时");
                    return true;
                }
                // 再添加一个打不开的超时检测
    ...
    循环执行(操作);
    // 多此一举 if(result && typeof result.text == "function") return result.text();
    return result;
{/* 
    for (let i = 0; wait && i < 50; i++) {
        // 不知道要不要注释掉
...
function reinstallTikTok(){
    let ua = uninstallTiktok()
    let ia = installTiktok()
...
function 使用应用变量(){
    return true;
    let t = 300, pkg = "com.sollyu.xposed.hook.model";
    let 操作 = [
        step(
            "打开appEnv"
            , function(){return !(packageName(pkg).findOne(1000))}
            , function(){
                if(launch(pkg)){
                    sleep(2000)
                } else {
        ...
        , step(
            "首页检测"
            , function(){return text("AppEnv").packageName(pkg).findOne(t);}
            , function(){
                let uo = text(appPackage).findOne(t);
                if(uo){
                    clickOn(uo.parent().parent())
                } else {
                    log("找不到软件", appPackage)
                }
            }
        )
        , step(
            "打开按钮"
            , function(){
                return (!(text("Run App").findOne(t)
                        && text("Save").findOne(t)))
                    && text("build").packageName(pkg).findOne(t)
            }
            , function(){
                clickOn(className("android.widget.ImageButton")
                    .boundsInside(device.width*0.5, device.height*0.5, device.width, device.height))
            }
        )
        , step(
            "Run App"
            , function(){return (this.uo = text("Run App").findOne(t))}
            , function(){
                // 
                log("Save", clickBut(text("Save").findOne(t)))
                log("Wipe Data", clickBut(text("Wipe Data").findOne(t)))
                let runAppClick = clickBut(text("Run App").findOne(t))
                log("Run App", runAppClick)
                if(runAppClick) {
                    return "跳出循环执行";
                }
            }
    ...
    function clickBut(uo){
        if(!uo || !uo.bounds) {
            tlog("不能点击", uo)
            return false;
        }
        let rect = uo.bounds()
        return clickOn(className("android.widget.ImageButton")
            .boundsInside(rect.left, rect.top-30, device.width, rect.bottom+30))
    }
}
