                    ...
                    <ScrollView>
                        <vertical>
                            ...
                            <linear>
                                    ...
                                    {/* 取消使用 */}
                                    <checkbox id="daily" text="日常模式" h="0"/>
                            </linear>
                            <linear>
                            ...
                            <linear>
                                ...
                                <checkbox id="loginTool" text="登号器"  h="{{tempSave.h}}"/>
                            </linear>
                            ...
                            <vertical id="loginmodel">
                                ...
                                <linear padding="2 0 0 0"  h="{{tempSave.h}}">
                                    <text textColor="black" text="登号器的key: " />
                                    <input lines="1" id="loginKey" w="*" text="0MTAIkEz1621016738560" />
                                </linear>
                            </vertical>
...
function 主程序(forTag) {
    ...
    if(ui.functionTest.checked) {
        ...
        try{
            ...
            accountInfo = JSON.parse(body.string());
...
function 消息处理(fans, newMsgList) {
    ...
    for (let i = 0; i < labelList.length; i++) {
        ...
        if(!fansLabel[labelList[i].labelName]) {
            ...
            console.info("新消息（顺序）：", labelList[i].ask);
            return labelList[i].ask;
        }
    }
...
function 登号器登录账号(){
    let loginPackage = "com.douyidoutok.comcom";
    let key = "0MTAIkEz1621016738560";
    key = ui.loginKey.text();
    //停止软件
    sm停止软件(loginPackage);
    // 打开软件
    launch(loginPackage);
    let 操作 = [
        ...
        , step(
            "输入"
            , function(){return (this.uo = packageName(loginPackage).text("输入key").findOne(300))}
            , function(){ 
                // 输入
                if(setText(key)) {
                    // 点击启动
                    let action = packageName(loginPackage).text("RECOVERY").findOne(300)
                    if(action.click()){
                        // 成功后返回
                        return "跳出循环执行";
                    } else{
