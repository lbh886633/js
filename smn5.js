    ...
    <button id="modeSelection" textColor="black" w="*" gravity="center" textSize="20" 
        ...
        {/* text="模式选择"/> */}
                            ...
                            <linear>
                                ...
                                <checkbox id="sendEmojiMsg" text="发消息带表情" />
                            </linear>
...
function 消息处理(fans, newMsgList) {
    ...
    for (let i = 0; i < labelList.length; i++) {
        ...
        if(!fansLabel[labelList[i].labelName]) {
            ...
            let msg = labelList[i].ask.split(",");
            msg = msg[random(0, msg.length-1)];
...
function dfs(words, wait) {
    let result = 0, t = 300, maxTime = Date.now(), stepTime = 1000*30;
    let 操作 = [
        step(
            "获取状态"
            , function(){
                ...
                if(1 == id("c_e").className("android.widget.TextView").clickable(true).find().length){
                    // 6秒钟...
                    if(stepTime/5 < Date.now()-maxTime) {
                        console.error("错误的链接");
                        return true;
                    }
                }
        ...
        , step(
            "Open App || ALWAYS || 打开"
            , function(){return (this.uo = text("Open App").findOne(t) 
                                        || text("ALWAYS").findOne(t) 
                                        || text("打开").findOne(t))}
            , function(){
                sleep(100); // 慢一点...
                clickOn(this.uo);
...
function 登号器登录账号(switchVPN, key){
    ...
    if(!tempSave.accountKey.keyName) {
