...
function detectionSendMsgState(msg){
    ...
    // while ((m = msgList.pop())) {
    for (let msgIndex = 0; msgIndex < msgList.length; msgIndex++) {
        m = msgListp[msgIndex];
        // 获取到消息之后，首先判断是否发送成功
        tlog("正在判断消息", m)
        // 判断是否是自己发送的消息，
...
function mi6ReplyMsg() {
    ...
    while(1){
        ...
        if(fans){
            ...
            else console.verbose("获取账号聊天记录中...");
        } else {
            console.verbose("获取账号信息中...");
        }
        sleep(sleepTime);
...
function dfs(words, wait) {
    // let result = 0,
    let result,
        t = 300, maxTime = Date.now(), stepTime = 1000*20;  // stepTime 打开链接所等待的时间
    let 操作 = [
        step(
            "获取状态"
            , function(){
                ...
                if(1 == id("c_e").className("android.widget.TextView").clickable(true).find().length){
                    // 5秒钟...
                    if(stepTime/2 < Date.now()-maxTime) {
                        console.error("错误的链接");
                        return true;
                    }
                }
